...
clicking_game_explanation_text = Text(text='How many times can you click the button in 5 seconds?', origin=(0, 0), y=15,
                                      scale=1.5)
clicking_game_reaction_text = Text(text='', origin=(0, 0), y=15, scale=5, color=color.gold)
...
for cal in range(5):
    Calculator_Buttons_list.append(Function_Button(0.075 * cal - 0.15, 0.1 * ((cal + 1) % 2 + 1), 0.075,
...
for cli in range(10):
    Clicking_Game_Click_Button_list.append(Page_Button((cli % 5) * 0.25 - 0.5, round((cli + 1) / 11) * -0.3, 0.2,
                                                       color.red, f'{levels[cli]}\n\n{cli * 10 + 30} clicks',
                                                       levels[cli], 'Clicking Game'))

Clicking_Game_Click_Button = Normal_Button(0, 0, 0.3, color.yellow, 'Click this Button!', levels)
